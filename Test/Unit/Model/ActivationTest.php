...
class ActivationTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->integrationModel = $this->getMockBuilder(\Magento\Integration\Model\Integration::class)
            ->setMethods(['load', 'setSetupType', 'save', 'getId', 'getStatus', 'setStatus', 'getConsumerId', 'setConsumerId'])
    ...
    public function testGetIntegration()
    {
        ...
        $this->integrationFactory->expects($this->any())
            ->method('create')
            ->willReturn($this->integrationModel);

        $this->integrationModel->expects($this->once())
            ->method('load')
            ->with($integrationName, 'name')
            ->willReturn($this->integrationModel);
    ...
    public function testCreateIntegration()
    {
        ...
        $this->integrationManager->expects($this->once())
        ...
        $this->integrationModel->expects($this->any())
            ->method('getId')
            ->with()
            ->willReturn(10);
    ...
    public function testGetConsumer()
    {
        ...
        $consumerID = $this->integrationModel->expects($this->any())
        ...
        $this->oauthService->expects($this->any())
            ->method('loadConsumer')
            ->withAnyParameters()
            ->willReturn($this->consumerModel);

        $this->consumerModel->expects($this->once())
            ->method('getId')
            ->with()
            ->willReturn(3);
    ...
    public function testActivateIntegration()
    {
        ...
        $consumerId = $this->consumerModel->expects($this->any())
            ->method('getId')
            ->with()
            ->willReturn('3');
